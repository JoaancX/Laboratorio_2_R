...
flights_selected <- flights%>%
  select(hour,origin,dest,tailnum,carrier,arr_delay,dep_delay)
...
```{r}
precip_delay <-inner_join(flights,weather)%>%
  group_by(precip)%>%
...
flights_airports <- inner_join(airports,fae, by = "delay")
...
```{r}
ggplot(flights_airports, aes(x=lom, y=lat, color=delay, size=delay)) +
  geom_point()
